...
print("[결측치 현황]")
print(df.isnull().sum())
print(f"총 결측치: {df.isnull().sum().sum()}개 ({df.isnull().sum().sum()/(df.shape[0]*df.shape[1])*100:.2f}%)")
print()
...
for idx, col in enumerate(numeric_cols):
    ...
    axes[idx].axvline(data.mean(), color='red', linestyle='--', label=f'평균: {data.mean():.2f}')
    axes[idx].axvline(data.median(), color='green', linestyle=':', label=f'중앙값: {data.median():.2f}')
...
plt.tight_layout()
plt.savefig('01_univariate_numeric.png', dpi=150)
plt.show()

print("'01_univariate_numeric.png' 저장 완료")
print()
...
axes[0].set_title('라인별 데이터 수')
axes[0].set_xlabel('라인')
axes[0].set_ylabel('빈도')

df['시간대'].value_counts().plot(kind='bar', ax=axes[1], color='coral', edgecolor='black')
axes[1].set_title('시간대별 데이터 수')
axes[1].set_xlabel('시간대')

plt.tight_layout()
plt.savefig('02_univariate_categorical.png', dpi=150)
plt.show()

print("'02_univariate_categorical.png' 저장 완료")
print()
...
print("'03_correlation_heatmap.png' 저장 완료")
print()
...
# 라인별 불량률
line_defect = df.groupby('라인')['불량률'].agg(['mean', 'std'])
axes[0].bar(line_defect.index, line_defect['mean']*100, yerr=line_defect['std']*100,
...
# 시간대별 불량률
time_defect = df.groupby('시간대')['불량률'].agg(['mean', 'std'])
axes[1].bar(time_defect.index, time_defect['mean']*100, yerr=time_defect['std']*100,
            capsize=5, color=['coral', 'steelblue'], alpha=0.7, edgecolor='black')
axes[1].set_title('시간대별 평균 불량률')
axes[1].set_ylabel('불량률 (%)')
axes[1].set_xlabel('시간대')

plt.tight_layout()
plt.savefig('04_bivariate_categorical.png', dpi=150)
plt.show()

print("'04_bivariate_categorical.png' 저장 완료")
print()

# 온도 vs 불량률 산점도
plt.figure(figsize=(10, 6))
plt.scatter(df['온도'].dropna(), df['불량률'], alpha=0.3, c='steelblue')
# 추세선
z = np.polyfit(df['온도'].dropna(), df.loc[df['온도'].notna(), '불량률'], 1)
p = np.poly1d(z)
temp_sorted = np.sort(df['온도'].dropna())
plt.plot(temp_sorted, p(temp_sorted), 'r--', linewidth=2, label='추세선')
plt.xlabel('온도 (°C)')
plt.ylabel('불량률')
...
plt.tight_layout()
plt.savefig('05_temp_vs_defect.png', dpi=150)
plt.show()
...
print(f"온도-불량률 상관계수: {r:.4f}")
print()
print("'05_temp_vs_defect.png' 저장 완료")
print()
...
plt.ylabel('라인')
plt.tight_layout()
plt.savefig('06_pivot_heatmap.png', dpi=150)
plt.show()

print("'06_pivot_heatmap.png' 저장 완료")
print()
...
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('07_monthly_trend.png', dpi=150)
plt.show()

print("'07_monthly_trend.png' 저장 완료")
print()
...
print(f"[온도 이상치]")
print(f"정상 범위: [{Q1_temp - 1.5*IQR_temp:.1f}, {Q3_temp + 1.5*IQR_temp:.1f}]")
...
if p_value < 0.05:
    print("→ 라인 B의 불량률이 통계적으로 유의미하게 높음")
print()
...
3. 오후 시간대 품질 관리
   ...
   - 환경 조건 재검토
""".format(
    (df[df['라인']=='B']['불량률'].mean() - df['불량률'].mean()) * 100,
    (df[df['시간대']=='오후']['불량률'].mean() - df[df['시간대']=='오전']['불량률'].mean()) * 100,
    r
))
...
plt.tight_layout()
plt.savefig('08_eda_dashboard.png', dpi=200, bbox_inches='tight')
plt.show()

print("'08_eda_dashboard.png' 저장 완료")
print()

print("=" * 60)
print("10차시 실습 완료!")
print("Part II 완료! 수고하셨습니다!")
print("=" * 60)
