...
| 번호 | 학습 목표 | 핵심 역량 |
|:----:|----------|----------|
| 1 | 시간에 따라 변하는 데이터의 특성을 이해함 | 시계열 개념 이해 |
| 2 | 시계열 특성 엔지니어링을 수행함 | 데이터 전처리 역량 |
| 3 | ML 모델로 시계열 데이터를 예측함 | 예측 모델링 역량 |
| 4 | 데이터 누출을 방지하는 올바른 분할 방법을 적용함 | 검증 설계 역량 |
...
| 데이터셋 | 출처 | 설명 | 용도 |
|---------|------|------|------|
| AirPassengers | Box-Jenkins (1976) | 1949-1960년 월별 항공 승객 수 | 시계열 분석 실습 |

- 총 144개 관측치 (12년 x 12개월)
- 명확한 상승 추세와 계절성 패턴 존재
- 시계열 분석의 대표적 벤치마크 데이터셋
...
**실제 데이터 계산 예시 (AirPassengers, 1949년)**

| 월 | 승객 수 ($X_t$) | 3개월 이동평균 ($MA_3$) | 계산 과정 |
|:--:|:--------------:|:----------------------:|----------|
| 1월 | 112 | - | 데이터 부족 |
| 2월 | 118 | - | 데이터 부족 |
| 3월 | 132 | 120.7 | $(112 + 118 + 132) / 3$ |
| 4월 | 129 | 126.3 | $(118 + 132 + 129) / 3$ |
| 5월 | 121 | 127.3 | $(132 + 129 + 121) / 3$ |
| 6월 | 135 | 128.3 | $(129 + 121 + 135) / 3$ |
...
**실제 데이터 계산 예시 (AirPassengers, $\alpha = 0.3$)**

| 월 | 승객 수 ($X_t$) | 지수평활 ($S_t$) | 계산 과정 |
|:--:|:--------------:|:---------------:|----------|
| 1월 | 112 | 112.0 | 초기값 = $X_1$ |
| 2월 | 118 | 113.8 | $0.3 \times 118 + 0.7 \times 112$ |
| 3월 | 132 | 119.3 | $0.3 \times 132 + 0.7 \times 113.8$ |
| 4월 | 129 | 122.2 | $0.3 \times 129 + 0.7 \times 119.3$ |
| 5월 | 121 | 121.8 | $0.3 \times 121 + 0.7 \times 122.2$ |
| 6월 | 135 | 125.8 | $0.3 \times 135 + 0.7 \times 121.8$ |
...
**AirPassengers 데이터의 자기상관 해석**

| 시차 (Lag) | 자기상관 ($\rho_k$) | 해석 |
|:----------:|:------------------:|------|
| 1 | 0.95 | 이전 달과 매우 강한 상관관계 |
| 6 | 0.80 | 6개월 전과 강한 상관관계 |
| 12 | 0.76 | 1년 전(동월)과 강한 상관관계 - 계절성 |
| 24 | 0.60 | 2년 전과 중간 수준 상관관계 |
...
| 파트 | 주제 | 핵심 내용 | 시간 |
|:----:|------|----------|:----:|
| 1 | 시계열 데이터의 이해 | 시간 의존성, 계절성, 추세, datetime 처리 | 8분 |
| 2 | 시계열 특성 엔지니어링 | Lag 특성, Rolling 특성, 날짜 특성 | 8분 |
| 3 | ML 기반 예측 및 평가 | RandomForest 예측, MAE/RMSE/MAPE 평가, TimeSeriesSplit | 9분 |
...
| 구성요소 | 설명 | AirPassengers 예시 |
|---------|------|-------------------|
| 추세 (Trend) | 장기적인 증가/감소 경향 | 1949년 112명 -> 1960년 432명 상승 |
| 계절성 (Seasonality) | 일정 주기로 반복되는 패턴 | 매년 7-8월(여름) 승객 급증 |
| 잔차 (Residual) | 추세와 계절성으로 설명 안 되는 부분 | 불규칙 변동, 노이즈 |
...
| 비정상성 | 평균/분산이 시간에 따라 변함 | AirPassengers: 변동폭이 점점 증가 |
...
| 교차검증 방법 | K-Fold | TimeSeriesSplit |
| 특성 생성 | 독립적 계산 | 과거 데이터만 사용 |
...
**차트 해석 방향:**
1. **원본 시계열**: 상승 추세와 주기적 패턴(계절성) 확인
2. **연도별 총 승객 수**: 매년 증가하는 추세 파악
3. **월별 평균**: 7-8월에 피크, 11월에 저점 - 계절성 패턴
4. **이동평균**: 노이즈 제거 후 명확한 추세선 확인
...
| Lag 특성 | 과거 값 | lag_1, lag_12 | shift() 사용 필수 |
| Rolling 특성 | 이동 통계 | ma_3, ma_12, std_12 | shift() 먼저 적용 |
...
**순환 인코딩의 필요성:**
- 일반 정수 인코딩: 12월(12)과 1월(1)이 멀게 인식됨
- sin/cos 인코딩: 12월과 1월이 가깝게 인식됨 (원형 좌표계)
...
| 월 | 승객 수 | wrong_ma_3 | correct_ma_3 |
|:--:|:------:|:----------:|:------------:|
| 4월 | 129 | 126.3 | 120.7 |
| 5월 | 121 | 127.3 | 126.3 |
| 6월 | 135 | 128.3 | 127.3 |

- wrong_ma_3: 현재 값(예: 6월 135)이 평균에 포함됨 (미래 정보 누출)
- correct_ma_3: 과거 값(3-5월)만 사용하여 계산됨 (안전)

---

### 2.7 데이터 누출 방지 체크 흐름도

```mermaid
flowchart TD
    A[특성 생성 코드 작성] --> B{shift 사용 여부 확인}

    B -->|Lag 특성| C{shift(n) 사용?}
    C -->|Yes| D[안전]
    C -->|No| E[미래 누출 위험!]

    B -->|Rolling 특성| F{shift(1) 먼저 적용?}
    F -->|Yes| G[안전]
    F -->|No| H[미래 누출 위험!]

    B -->|변화 특성| I{shift된 값으로 계산?}
    I -->|Yes| J[안전]
    I -->|No| K[미래 누출 위험!]
...
| 카테고리 | 특성 | 개수 |
|---------|------|:----:|
| 날짜 특성 | year, month, quarter, sin/cos 인코딩, time_idx | 8개 |
| Lag 특성 | lag_1, lag_2, lag_3, lag_6, lag_12 | 5개 |
| Rolling 특성 | ma_3/6/12, std_3/6/12, max_12, min_12, range_12 | 9개 |
| 변화 특성 | diff_1, diff_12, pct_1, yoy_ratio | 4개 |
| **합계** | | **26개** |
...
flowchart LR
    ...
    subgraph SPLIT["데이터 분할"]
        C --> D[시간 기준 분할]
        D --> E[학습 세트<br/>과거 데이터]
        D --> F[테스트 세트<br/>미래 데이터]
    end

    subgraph MODEL["모델링"]
        E --> G[모델 학습<br/>RandomForest]
        G --> H[예측]
        F --> H
    end

    subgraph EVAL["평가"]
        H --> I[평가 지표<br/>MAE, RMSE, MAPE]
        I --> J[특성 중요도<br/>분석]
        J --> K[결과 시각화]
    end
...
### 3.3 TimeSeriesSplit vs K-Fold 비교

| 구분 | K-Fold | TimeSeriesSplit |
|------|--------|-----------------|
| 분할 방식 | 랜덤 분할 | 시간 순서 유지 |
| 학습 데이터 | 각 Fold에서 고정 비율 | Fold가 진행될수록 증가 |
| 검증 데이터 | 랜덤 선택 | 항상 학습 데이터 이후 시점 |
| 미래 정보 사용 | 가능 (데이터 누출 위험) | 불가능 (안전) |
| 적용 대상 | 독립적 관측치 | 시계열 데이터 |
| sklearn 클래스 | `KFold` | `TimeSeriesSplit` |

**시각적 비교:**

```
[K-Fold (5-Fold)]
Fold 1: [V V V] [T T T T T T T T T T T T]
Fold 2: [T T T] [V V V] [T T T T T T T T T]
Fold 3: [T T T T T T] [V V V] [T T T T T T]
...
V = Validation, T = Train (무작위 선택)

[TimeSeriesSplit (5-Fold)]
Fold 1: [T T T T T T T T] [V V]
...
T = Train (과거), V = Validation (미래) - 시간 순서 유지
```

---

### 3.4 실습: RandomForest 모델 학습
...
| 지표 | 공식 | 특징 | 해석 기준 |
|------|------|------|----------|
| MAE | $\frac{1}{n}\sum\|y - \hat{y}\|$ | 해석 쉬움, 단위 동일 | 작을수록 좋음 |
| RMSE | $\sqrt{\frac{1}{n}\sum(y - \hat{y})^2}$ | 큰 오차에 패널티 | 작을수록 좋음 |
| MAPE | $\frac{100}{n}\sum\frac{\|y - \hat{y}\|}{y}$ | 상대적 오차 (%) | 10% 미만 우수 |
| R^2 | $1 - \frac{SS_{res}}{SS_{tot}}$ | 설명력 (0~1) | 1에 가까울수록 좋음 |
...
| 순위 | 특성 | 중요도 | 해석 |
|:----:|------|:------:|------|
| 1 | lag_12 | 0.35 | 전년 동월 영향 큼 (계절성) |
| 2 | lag_1 | 0.20 | 직전 월 영향 큼 (자기상관) |
| 3 | ma_12 | 0.12 | 1년 이동평균 (추세) |
| 4 | time_idx | 0.08 | 시간 흐름 (장기 추세) |
| 5 | month | 0.07 | 월별 계절성 |
...
**차트 해석 방향:**
1. **실제 vs 예측**: 두 선이 가까울수록 좋은 예측, 95% 구간 내 포함 확인
2. **특성 중요도**: 어떤 특성이 예측에 가장 기여하는지 파악
3. **잔차 분포**: 0 중심의 정규분포에 가까울수록 좋음
4. **산점도**: 대각선에 가까울수록 예측 정확도 높음

---

### 3.11 실습: TimeSeriesSplit 교차검증
...
| 변화 특성 | `df['value'].shift(1) - df['value'].shift(2)` | `df['value'].diff(1)` (주의 필요) |
