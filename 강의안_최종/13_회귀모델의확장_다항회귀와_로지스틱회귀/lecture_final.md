...
| 목표 | 설명 |
|------|------|
| 다항회귀 모델링 | 비선형 관계를 다항 특성 변환으로 모델링함 |
| 분류 문제 이해 | 회귀와 분류의 차이를 구분하고 시그모이드 함수를 이해함 |
| 로지스틱 회귀 | LogisticRegression으로 이진 분류 모델을 생성하고 오즈비를 해석함 |
...
#### 실제 데이터 예시: 마력(Horsepower)과 연비(MPG)

Auto MPG 데이터셋의 2차 다항회귀 결과:

$$MPG = 56.90 + (-0.4667) \times HP + (0.001230) \times HP^2$$

구체적인 계산 예시:

| 마력(HP) | 선형항 | 2차항 | 예측 MPG |
|----------|--------|-------|----------|
| 80 | $-0.4667 \times 80 = -37.34$ | $0.00123 \times 80^2 = 7.87$ | $56.90 - 37.34 + 7.87 = 27.43$ |
| 100 | $-0.4667 \times 100 = -46.67$ | $0.00123 \times 100^2 = 12.30$ | $56.90 - 46.67 + 12.30 = 22.53$ |
| 150 | $-0.4667 \times 150 = -70.01$ | $0.00123 \times 150^2 = 27.68$ | $56.90 - 70.01 + 27.68 = 14.57$ |
| 200 | $-0.4667 \times 200 = -93.34$ | $0.00123 \times 200^2 = 49.20$ | $56.90 - 93.34 + 49.20 = 12.76$ |

해석:
- 마력이 증가할수록 연비가 감소하는 비선형 관계
- 선형항($\beta_1 < 0$)이 연비 감소의 주 원인
- 2차항($\beta_2 > 0$)이 높은 마력에서 감소 속도를 완화
...
**해석 방향**:
- 세 그래프 모두 데이터가 직선보다는 곡선 형태로 분포함
- 마력, 무게, 배기량이 증가할수록 연비가 감소하는 음의 관계
- 감소 속도가 점차 완만해지는 비선형 패턴 관찰
...
**해석 방향**:
- 빨간색 직선(선형회귀)은 데이터의 곡선 패턴을 포착하지 못함
- 녹색 곡선(2차 다항회귀)이 데이터 분포를 가장 잘 따름
- 파란색 곡선(3차 다항회귀)은 2차와 유사하나 차수가 높아 과대적합 위험
...
**해석 방향**:
- 파란선(학습 R^2)은 차수가 증가할수록 계속 상승
- 빨간선(테스트 R^2)은 차수 2에서 정점 후 하락
- 두 선의 간격이 벌어지는 지점에서 과대적합 발생
...
```mermaid
flowchart TD
    subgraph 모델선택["모델 선택 가이드"]
        A[문제 유형 확인] --> B{종속변수 유형?}
        B -->|연속형 숫자| C{관계 형태?}
        B -->|범주형 라벨| D[분류 문제]
        ...
        H --> J[로지스틱 회귀]
        I --> K["다항 로지스틱 회귀<br>(softmax)"]

        F --> L["PolynomialFeatures<br>+ LinearRegression"]
...
| z 값 | 계산식 | $\sigma(z)$ | 해석 |
|------|--------|-------------|------|
| -5 | $\frac{1}{1+e^{5}} = \frac{1}{1+148.41}$ | 0.0067 | 거의 0 (양품) |
| -2 | $\frac{1}{1+e^{2}} = \frac{1}{1+7.39}$ | 0.1192 | 낮은 확률 |
| 0 | $\frac{1}{1+e^{0}} = \frac{1}{1+1}$ | 0.5000 | 반반 |
| +2 | $\frac{1}{1+e^{-2}} = \frac{1}{1+0.135}$ | 0.8808 | 높은 확률 |
| +5 | $\frac{1}{1+e^{-5}} = \frac{1}{1+0.0067}$ | 0.9933 | 거의 1 (불량) |
...
| 확률 p | 계산식 | Odds | 해석 |
|--------|--------|------|------|
| 0.8 | $\frac{0.8}{0.2}$ | 4.0 | 발생이 4배 더 높음 |
| 0.5 | $\frac{0.5}{0.5}$ | 1.0 | 동등 |
| 0.2 | $\frac{0.2}{0.8}$ | 0.25 | 미발생이 4배 더 높음 |
| 0.9 | $\frac{0.9}{0.1}$ | 9.0 | 발생이 9배 더 높음 |
...
| 특성 | 계수 $\beta$ | 오즈비 $e^{\beta}$ | 해석 |
|------|-------------|-------------------|------|
| 온도 | 0.15 | $e^{0.15} = 1.16$ | 온도 1도 증가 시 불량 오즈 16% 증가 |
| 습도 | 0.08 | $e^{0.08} = 1.08$ | 습도 1% 증가 시 불량 오즈 8% 증가 |
| 압력 | -0.12 | $e^{-0.12} = 0.89$ | 압력 1단위 증가 시 불량 오즈 11% 감소 |
| 가동시간 | 0.05 | $e^{0.05} = 1.05$ | 가동시간 1시간 증가 시 불량 오즈 5% 증가 |

```mermaid
flowchart TD
    subgraph 학습흐름["로지스틱 회귀 학습 흐름"]
        A["입력 데이터<br>X1, X2, ..., Xn"] --> B["선형 결합<br>z = b0 + b1*X1 + b2*X2 + ..."]
        B --> C["시그모이드 변환<br>P = 1/(1+e^-z)"]
        C --> D{"P >= 0.5?"}
        D -->|Yes| E["예측: 클래스 1<br>(불량)"]
        D -->|No| F["예측: 클래스 0<br>(양품)"]

        E --> G["손실 함수 계산<br>(Cross-Entropy)"]
        F --> G
        G --> H["경사 하강법으로<br>가중치 업데이트"]
        H --> B
    end
...
**해석 방향**:
- S자 형태의 곡선으로 모든 입력을 0~1 사이로 변환
- z=0에서 0.5를 통과하며 대칭적인 형태
- z가 극단적으로 크거나 작으면 1 또는 0에 수렴
...
**해석 방향**:
- 왼쪽 그래프: 녹색 S자 곡선이 확률 변화를 표현
- 온도 85도 부근에서 확률이 0.5를 통과 (결정 경계)
- 결정 경계 좌측은 양품, 우측은 불량으로 분류
...
**해석 방향**:
- 대각선(TN, TP)의 값이 클수록 정확한 분류
- FN(불량->양품)이 0에 가까워야 제조업에서 안전함
- 색상이 진할수록 해당 셀의 빈도가 높음
...
**해석 방향**:
- 빨간색 막대: 계수가 양수, 해당 특성 증가 시 불량 확률 상승
- 파란색 막대: 계수가 음수, 해당 특성 증가 시 불량 확률 하락
- 막대 길이가 길수록 해당 특성의 영향력이 큼
...
| 항목 | 권장 사항 |
|------|----------|
| 다항회귀 차수 | 2~3차부터 시작, 교차검증으로 최적값 선택 |
| 로지스틱 회귀 | max_iter 충분히 설정 (1000 이상) |
| 불균형 데이터 | class_weight='balanced' 사용 |
| 임계값 조정 | 재현율 중시 시 0.3~0.4로 낮춤 |
